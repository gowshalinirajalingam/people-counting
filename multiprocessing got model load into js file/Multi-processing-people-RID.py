...
def inference_images_head(image1,class_file_path,saved_model_loaded,infer,interpreter,input_details,output_details,blur_intensity):
    try:
...
def python_function(msg_py,msg_local_py,current_process,exp_process,completed_process,msg_passing,people_params_l,head_params_l,model_RID_l):
        ...
        people_params = people_params_l[0]
        head_params = head_params_l[0]
        model_RID = model_RID_l[0]

        
        infer_people = people_params["saved_model_loaded"], people_params["infer"]
        interpreter_people, input_details_people = people_params["interpreter"], people_params["input_details"]
        output_details_people = people_params["output_details"]
        
        saved_model_loaded_head,infer_head = head_params["saved_model_loaded"], head_params["infer"]
        interpreter_head, input_details_head = head_params["interpreter"], head_params["input_details"]
        output_details_head = head_params["output_details"]
        ...
        if msg_local_py is None:
            ...
            try:
                ...
                while True:
                        ...
                        if framework == 'tflite':
                            interpreter_people.set_tensor(input_details_people[0]['index'], image_data)
                            interpreter_people.invoke()
                            pred = [interpreter_people.get_tensor(output_details_people[i]['index']) for i in range(len(output_details_people))]
                            ...
                            batch_data = tf.constant(image_data)
                            pred_bbox = infer_people(batch_data)
                            for key, value in pred_bbox.items():
                                boxes = value[:, :, 0:4]
                                pred_conf = value[:, :, 4:]
                        ...
                        if blur_intensity != 0:  #0 means no need to blur the face
                            frame,start_time,end_time,scores,classes = inference_images_head(frame, class_file_path, saved_model_loaded_head,infer_head, interpreter_head, input_details_head, output_details_head, blur_intensity) #inference
                        ...
                        for track in tracker.tracks:
                            ...
                            try:
                               ...
                               if dd[1] >2 and dd[1] <12 :   #check similarity for a object only 10 times
                                    ...
                                    ###SKIP comparing the image with same trackid images and it's partners. so that move them to temp folder when not needed to compare the image with itself and its partners.
                                    if len(g_dirs)>0:
                                        for p in partners:
                                            if p in g_dirs:
